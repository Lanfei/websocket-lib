...
function Server(options, sessionListener) {
	...
	server.on('upgrade', function (req, socket) {
		...
		var upgradeHeader = req.headers['upgrade'];
...
ServerSession.prototype.accept = function () {
	...
	var subProtocol = reqHeaders['sec-websocket-protocol'];
	...
	if (subProtocol && !resHeaders['Sec-WebSocket-Protocol']) {
		headers['Sec-WebSocket-Protocol'] = subProtocol.split(/, +/)[0];
	}
